...
int main(int argc, char **argv) {
    ...
    if (port < 0) port = sc.rpc_user_port;
    ...
    if (!srv) { MK_LOG_ERROR("Failed to create RPC server"); miku_user_service_destroy(svc); return 1; }
    ...
    if (miku_rpc_server_start(srv) != 0) {
        ...
        miku_user_service_destroy(svc); return 1;
    }
